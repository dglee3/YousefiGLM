...
datmat = 'jn029-11';
[Data, D, W, samps] = generateCovariates(datmat);
...
%         figure(1)
        ...
        e = errorbar(6.4,B(14),2*stat.se(14));
        hold on
        e.Marker = 'square';
        e.MarkerSize = 10;
        e.Color = [1 0.5 0];
...
%         pause()
        im_filepath = ['./GLM Images/ay_glm_fit_YA_licks/' datmat];
        if exist(im_filepath,'dir') ~= 7
            mkdir(im_filepath);
        end
        saveas(gcf,[im_filepath '/Cell ' num2str(i)],'tiff')
        close all
        clc
    end
