...
datmat = 'jn029-10';
[Data, D, W, len] = generateCovariates(datmat);
% important indexces
% len = 340;
...
%         e.LineWidth = 0.2;
        ...
        axis tight
        title(['Cell ' num2str(i)],'FontSize',24)
        xlabel('Index')
        ylabel('Coef.','FontSize',18)
        ...
        xlim([0 12])
        ylim(ylimDims)
        ylabel('norm-count','FontSize',18)
        legend({'cw-cw'},'FontSize',18)
...
%         patch([1:len len:-1:1]',[Yt_low(3*len+(1:len)); Yt_high(3*len+(len:-1:1))],'k')
        ...
        saveas(gcf,['./GLM Images/' datmat '/Cell ' num2str(i)],'tiff')
        close all
        clc
%         pause()
    end
